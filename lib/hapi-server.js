    ...
    if (typeof internals.settings.plugins === 'object' || typeof internals.settings.plugins === 'array') {
        Async.map(internals.settings.plugins, function(plugin, callback) {
            ...
            try {
                ...
                return callback('Hapi plugin "' + plugin.name + '" not found');
            }

            // Add plugin to server
            plugin.settings = plugin.settings || null;
            server.pack.require(plugin.name, plugin.settings, function(err) {
                if (err) {
                    return callback('Problem loading Hapi plugin "' + plugin.name + '"');
                }
                return callback(null, plugin.name);
            });
