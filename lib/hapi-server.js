module.exports = function(docpad, settings, port, hostname) {
    ...
    var readDatabase = function(request, reply) {
        ...
        internals.serveDocument = function(document) {
            ...
            if (content) {
                ...
                var lastModified = document.get('mtime').toUTCString();

                return reply(content).header('content-type', contentType).header('ETag', '"' + stat.size + '-' + Number(mtime) + '"').ttl(internals.maxAge).header('last-modified', lastModified).takeover();
            }
            else {
                return reply();
            }
    ...
    internals.staticPath = process.env.NODE_ENV === 'production' ? 'public_html/' : 'out/';
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        server.ext('onPreResponse', function (request, reply) {
            ...
            if (request.route.path === '/{file*}' && error.output.statusCode === 404) {
